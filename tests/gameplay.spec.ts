...
test.skip('should detect immobile state and show a message about restarting/undoing', () => { /* TODO */ })
...
test('game should be beatable (using recorded playthroughs)', async ({ page }) => {
  ...
  for (let i = 0; i < levels.length; i++) {
    ...
    console.log(`Playing level ${levelId} with ${moves.length} moves: ${moves.map((move) => typeof move === 'object' ? `click (${move.click.x}, ${move.click.y})` : move).join(', ')}`)
